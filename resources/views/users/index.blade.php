...
<div class="px-4 sm:px-6 lg:px-8">
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div class="sm:flex-auto">
            <h1 class="text-4xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Users</h1>
    ...
    <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200/50 p-6 mb-6">
        <form method="GET" action="{{ route('users.index') }}" class="flex gap-3 flex-wrap">
                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-indigo-50/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium space-x-3">
                                ...
                                @if($user->id !== auth()->id())
                                    <button type="button" 
                                            onclick="openDeleteUserModal({{ $user->id }}, '{{ $user->name }}', '{{ $user->username }}')"
